...
def create_instance(name, ami=aws_cfg["ubuntu_lts_ami"],
    ...
    if raw_input("Add to ssh/config? (y/n) ").lower() == "y":
        add_to_ssh_config(name, instance.public_dns_name, key_name,
                          key_extension, key_dir)
...
def terminate_instance(name):
    ...
    for reservation in conn.get_all_instances(filters=filters):
        for instance in reservation.instances:
            ...
            if raw_input("Delete from ssh/config? (y/n) ").lower() == "y":
...
def stop_instance(name):
    """
    Stop all servers with the given name
    """

    print(_green("Started stopping {}...".format(name)))

    conn = connect_to_ec2()
    filters = {"tag:Name": name}
    for reservation in conn.get_all_instances(filters=filters):
        for instance in reservation.instances:
            if "stopped" in str(instance._state):
                print "instance {} is already stopped".format(instance.id)
                continue
            if "terminated" in str(instance._state):
                print "instance {} has been terminated".format(instance.id)
                continue
            else:
                print instance._state
            print (instance.id, instance.tags['Name'])
            if raw_input("stop? (y/n) ").lower() == "y":
                print(_yellow("Stopping {}".format(instance.id)))
...
def start_instance(name,
                   ...
                   key_dir='~/.ec2'):
    """
    Start all servers with the given name
    """

    print(_green("Starting {}...".format(name)))

    conn = connect_to_ec2()
    filters = {"tag:Name": name}
    for reservation in conn.get_all_instances(filters=filters):
        for instance in reservation.instances:
            if "running" in str(instance._state):
                print "instance {} is already running".format(instance.id)
                continue
            if "terminated" in str(instance._state):
                ...
                print instance._state
            print (instance.id, instance.tags['Name'])
            if raw_input("start? (y/n) ").lower() == "y":
                print(_yellow("Starting {}".format(instance.id)))
                conn.start_instances(instance_ids=[instance.id])
                print(_yellow("Started"))
...
def add_to_ssh_config(name,
    ...
    """.format(name=name,
               dns=public_dns_name,
               key_file_path=os.path.join(os.path.expanduser(key_dir),
